...
@implementation VRVIUPlayerHistoryItem
...
@interface VRVIUPlayerHistory ()

@end

@implementation VRVIUPlayerHistory {
    NSMutableArray *_list;
}

+ (instancetype)instance {
    static VRVIUPlayerHistory *s_obj = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        s_obj = [[VRVIUPlayerHistory alloc] init];
    });
...
- (NSString *)dbfilePath {
    ...
    libraryPath = [libraryPath stringByAppendingPathComponent:@"vrviuplayerhistory.plist"];
...
- (void)add:(VRVIUPlayerHistoryItem *)item {
    __block NSUInteger findIdx = NSNotFound;
    
    [_list enumerateObjectsUsingBlock:^(VRVIUPlayerHistoryItem *enumItem, NSUInteger idx, BOOL *stop) {
